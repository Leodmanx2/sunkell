...
namespace sunkell {
	...
	namespace {
		...
		std::shared_ptr<key_event> translate_keyboard_input(const RAWINPUT& input) {
			// Union member access is safe if the tag is right
			...
			switch(scancode) {
				...
				case 0x51:
					if(E0) {
						return std::make_shared<key_event>(button::page_down, state);
					}
				...
				case 0x48:
					if(E0) {
						return std::make_shared<key_event>(button::arrow_up, state);
					}
				...
				case 0x38:
					if(E0) {
						return std::make_shared<key_event>(button::right_alt, state);
					}
	...
	input_event_queue::input_event_queue(const window* window) {
		HWND hWnd = window->platform_detail<HWND, window_tag>();
